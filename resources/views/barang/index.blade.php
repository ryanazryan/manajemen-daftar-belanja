            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <a href="{{ route('barang.create') }}">
                        <x-primary-button class="inline-block mb-4 px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">
                            Tambah Barang
                        </x-primary-button>
                    </a>
                    ...
                    <table class="min-w-full bg-white border border-gray-200">
                        <thead>
                            <tr class="bg-gray-100 text-gray-600 uppercase text-xs leading-normal">
                                ...
                                {{-- TAMBAHKAN HEADER TABEL UNTUK TANGGAL --}}
                                <th class="py-4 px-4 text-left whitespace-nowrap">Tanggal</th>
                                <th class="py-4 px-4 text-left whitespace-nowrap">Keterangan</th>
                        ...
                        <tbody class="text-gray-700 text-sm font-light divide-y divide-gray-200">
                            @foreach ($barangs as $barang)
                                <tr class="hover:bg-gray-100">
                                    ...
                                    <td class="py-4 px-4 text-left whitespace-nowrap">{{ $barang->nama_barang }}</td>
                                    <td class="py-4 px-4 text-left whitespace-nowrap">{{ $barang->nama_orang }}</td>
                                    <td class="py-4 px-4 text-center whitespace-nowrap">{{ $barang->kuantitas }}</td>
                                    ...
                                    {{-- TAMBAHKAN SEL DATA UNTUK TANGGAL --}}
                                    <td class="py-4 px-4 text-left whitespace-nowrap">
                                        {{ $barang->tanggal ? $barang->tanggal->translatedFormat('d M Y') : '-' }}
                                    </td>
                                    <td class="py-4 px-4 text-left whitespace-nowrap">{{ $barang->keterangan ?? '-' }}</td>
                                    <td class="py-4 px-4 text-center">
                                        <div class="flex items-center justify-center space-x-2">
                                            <a href="{{ route('barang.show', $barang->id) }}" class="p-2 bg-green-500 text-black rounded hover:bg-green-600">
                                            ...
                                            <a href="{{ route('barang.edit', $barang->id) }}" class="p-2 bg-yellow-500 text-black rounded hover:bg-yellow-600">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                                            ...
                                            <form action="{{ route('barang.destroy', $barang->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus barang ini?');">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="p-2 bg-red-500 text-black rounded hover:bg-red-600">
                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
